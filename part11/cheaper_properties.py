...
def cheaper_properties(properties,reference):
    return [(p,reference.price_difference(p)) for p in properties if (p.square_meters*p.price_per_sqm)<(reference.square_meters*reference.price_per_sqm)]
